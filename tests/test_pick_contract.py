...
def test_empty_snapshot_writes_explicit_status(tmp_path, monkeypatch):
    ...
    daily_pipeline._store_picks(
        {}, date(2026, 7, 22), status="no_qualifying_picks", notes="No edge."
    )
...
def test_export_preserves_no_games_status(tmp_path, monkeypatch):
    ...
    (processed / "picks_today.meta.json").write_text(
        json.dumps(
            {
                "status": "no_games",
    ...
    monkeypatch.setattr(
        export_best_bets, "OUT_PATH", tmp_path / "data_files" / "best_bets_today.json"
    )
...
def test_live_feature_builder_matches_model_schema(tmp_path, monkeypatch):
    ...
    required = set(
        daily_pipeline.MONEYLINE_FEATURES
        + daily_pipeline.SPREAD_FEATURES
        + daily_pipeline.TOTALS_FEATURES
    )
