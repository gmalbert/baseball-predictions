    ...
    for team in teams:
        ...
        try:
            ...
            hourly_df = pd.DataFrame(
                {
                    "time": times,
                    "temp_f": pd.to_numeric(
                        pd.Series(h.get("temperature_2m", []), dtype=object), errors="coerce"
                    ),
                    "wind": pd.to_numeric(
                        pd.Series(h.get("wind_speed_10m", []), dtype=object), errors="coerce"
                    ),
                    "wdir": pd.to_numeric(
                        pd.Series(h.get("wind_direction_10m", []), dtype=object), errors="coerce"
                    ),
                    "precip": pd.to_numeric(
                        pd.Series(h.get("precipitation", []), dtype=object), errors="coerce"
                    ),
                    "humid": pd.to_numeric(
                        pd.Series(h.get("relative_humidity_2m", []), dtype=object), errors="coerce"
                    ),
                    "cloud": pd.to_numeric(
                        pd.Series(h.get("cloud_cover", []), dtype=object), errors="coerce"
                    ),
                }
            )
            ...
            game_hours = hourly_df[
                (hourly_df["hour"] >= _GAME_HOUR_START) & (hourly_df["hour"] <= _GAME_HOUR_END)
            ]
            daily = (
                game_hours.groupby("date_day")
                .agg(
                    temp_f=("temp_f", "mean"),
                    wind_mph=("wind", "mean"),
